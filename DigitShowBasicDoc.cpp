...
CDigitShowBasicDoc::CDigitShowBasicDoc()
{
	...
	NUMDA=1;		// The Number of D/A Board ( NUMDA=1 )
	for(i=0;i<64;i++){
		Vout[i]=0.0;	
	...
	for(j=0;j<4;j++){
		...
		SpecimenData.VLDT1[j]=65.65;
		SpecimenData.VLDT2[j]=66.20;
	}
	SpecimenData.Gs=0.0;		
...
//	
	ControlData[1].MotorSpeed=200.0;	// Motor Speed in the control of "Pre-Consolidation"
	ControlData[1].q=10.0;				// Referential Stress in the control of "Pre-Consolidation"
	ERR_StressCom=1.0;					// (kPa)
	ERR_StressExt=-1.0;					// (kPa)
	ERR_StressA=1.0;					// (kPa)
	...
	DA_Cal_a[CH_EP_Cell]=0.011654252;			// D/A Calibration Factor, EP for Cell Pressure (V/kPa)
...
//	Specimen Data in drain and undrain condition
	...
	if(SpecimenData.VLDT1[0]>0.0 && Phyout[5]>0.0) {
		eLDT1=-log((SpecimenData.VLDT1[0]-Phyout[5])/SpecimenData.VLDT1[0])*100.0;	// True LDT Strain (%)
	}
	else{
		eLDT1=0.0;
	}
	if(SpecimenData.VLDT2[0]>0.0 && Phyout[6]>0.0) {
		eLDT2=-log((SpecimenData.VLDT2[0]-Phyout[6])/SpecimenData.VLDT2[0])*100.0;	// True LDT Strain (%)
	}
	else{
		eLDT2=0.0;
	}
	...
	case 15:
		{ 
			if( CURNUM >=0 && CURNUM < 128 ){
				...
				if( CFNUM[CURNUM]==8 )	CLoading_StrainLDT();
				DA_OUTPUT();
			}
		}
		break;
...
void CDigitShowBasicDoc::MLoading_Stress()
{
	...
	if(CFPARA[CURNUM][3] > 0.0) DAVout[CH_EP_Cell]=DAVout[CH_EP_Cell]-float(0.2*DA_Cal_a[CH_EP_Cell]*(e_sr-CFPARA[CURNUM][3]));
//
...
void CDigitShowBasicDoc::CLoading_Stress()
{
	...
	DAVout[CH_Motor]=5.0f;					// Motor: On
	DAVout[CH_MotorSpeed]=float(DA_Cal_a[CH_MotorSpeed]*CFPARA[CURNUM][1]+DA_Cal_b[CH_MotorSpeed]);	// Motor_Speed
	if(CFPARA[CURNUM][5] > 0.0) DAVout[CH_EP_Cell]=DAVout[CH_EP_Cell]-float(0.2*DA_Cal_a[CH_EP_Cell]*(e_sr-CFPARA[CURNUM][5]));
//
	if(CFPARA[CURNUM][0]==0.0){
		if(NUM_Cyclic==0){
			Flag_Cyclic=FALSE;
		...
		if(NUM_Cyclic!=0 && NUM_Cyclic <= CFPARA[CURNUM][4]){
			if(Flag_Cyclic==FALSE){
				DAVout[CH_MotorCruch]=0.0f;		// Cruch:Down
				if(q>=CFPARA[CURNUM][3]) Flag_Cyclic=TRUE;
			}
			if(Flag_Cyclic==TRUE){
				DAVout[CH_MotorCruch]=5.0f;		// Cruch:Up
				if(q<=CFPARA[CURNUM][2]) {
		...
		if(NUM_Cyclic!=0 && NUM_Cyclic <= CFPARA[CURNUM][4]){
			...
			if(Flag_Cyclic==TRUE){
				DAVout[CH_MotorCruch]=5.0f;		// Cruch:Up
				if(q<=CFPARA[CURNUM][2]) Flag_Cyclic=FALSE;
			}
		}
		if(NUM_Cyclic>CFPARA[CURNUM][4]){ 
			CURNUM=CURNUM+1;
...
//
	if(CFPARA[CURNUM][0]==0.0){
		...
		if(NUM_Cyclic!=0 && NUM_Cyclic <= CFPARA[CURNUM][4]){
			if(Flag_Cyclic==FALSE){
				DAVout[CH_MotorCruch]=0.0f;		// Cruch:Down
				if(ea>=CFPARA[CURNUM][3]) Flag_Cyclic=TRUE;
			}
			if(Flag_Cyclic==TRUE){
				DAVout[CH_MotorCruch]=5.0f;		// Cruch:Up
				if(ea<=CFPARA[CURNUM][2]) {
		...
		if(NUM_Cyclic!=0 && NUM_Cyclic <= CFPARA[CURNUM][4]){
			...
			if(Flag_Cyclic==TRUE){
				DAVout[CH_MotorCruch]=5.0f;		// Cruch:Up
				if(ea<=CFPARA[CURNUM][2]) Flag_Cyclic=FALSE;
			}
		}
		if(NUM_Cyclic>CFPARA[CURNUM][4]){ 
			CURNUM=CURNUM+1;
...
void CDigitShowBasicDoc::Creep()
	...
	// 2: time (min)
	// 3: sigma_r (kPa)
	// 4: Maximum value of error in q (kPa)
	// 5: Maximum motor speed (rpm)
{
	TotalStepTime=TotalStepTime+CtrlStepTime/60.0;
	DAVout[CH_Motor]=5.0f;		// Motor:On
	if(CFPARA[CURNUM][3] > 0.0) DAVout[CH_EP_Cell]=DAVout[CH_EP_Cell]-float(0.2*DA_Cal_a[CH_EP_Cell]*(e_sr-CFPARA[CURNUM][3]));
//
	if(q>=CFPARA[CURNUM][1]+ERR_StressCom)	{
		DAVout[CH_MotorCruch]=5.0f;	// Cruch:Up
		if(CFPARA[CURNUM][4] > 0.0){
			if(q > CFPARA[CURNUM][1]+CFPARA[CURNUM][4]){
				DAVout[CH_MotorSpeed]=float(DA_Cal_a[CH_MotorSpeed]*CFPARA[CURNUM][5]+DA_Cal_b[CH_MotorSpeed]);
			}
			if(q <= CFPARA[CURNUM][1]+CFPARA[CURNUM][4]){
				DAVout[CH_MotorSpeed]=float(DA_Cal_a[CH_MotorSpeed]*((q-CFPARA[CURNUM][1])/CFPARA[CURNUM][4])*CFPARA[CURNUM][5]+DA_Cal_b[CH_MotorSpeed]);
			}
		}
		else {
		...
		if(CFPARA[CURNUM][4] > 0.0){
			...
			if(q >= CFPARA[CURNUM][1]-CFPARA[CURNUM][4]){
				DAVout[CH_MotorSpeed]=float(DA_Cal_a[CH_MotorSpeed]*((CFPARA[CURNUM][1]-q)/CFPARA[CURNUM][4])*CFPARA[CURNUM][5]+DA_Cal_b[CH_MotorSpeed]);
			}
		}
		else {
			DAVout[CH_MotorSpeed]=float(DA_Cal_a[CH_MotorSpeed]*CFPARA[CURNUM][0]+DA_Cal_b[CH_MotorSpeed]);
...
void CDigitShowBasicDoc::Creep2()
	// Creep Loading without the Control in Extention Direction
	// 0: Motor_rpm_Fixed,
	// 1: q (kPa),
	// 2: time (min),
	// 3: sigma_r (kPa),
...
void CDigitShowBasicDoc::CLoading_StrainLDT()
	// Axial Cyclic Loading with Stain Control
	// 0: Compression:0.0/Extention:1.0,
	// 1: Motor_rpm, 
	// 2: LDT_strain_lower, 
	// 3: LDT_strain_upper,
	// 4: Number_cyclic
   ...
   // 6: con_pres
{
	...
	DAVout[CH_MotorSpeed]=float(DA_Cal_a[CH_MotorSpeed]*CFPARA[CURNUM][1]+DA_Cal_b[CH_MotorSpeed]);	// Motor_Speed
		...
		if(fabs(sr-CFPARA[CURNUM][6]) >= ERR_StressA){
			DAVout[CH_EP_Cell]=DAVout[CH_EP_Cell]-float(0.1*DA_Cal_a[CH_EP_Cell]*(sr-CFPARA[CURNUM][6]));	// e_sr ballanced
		}
	}
		else {
		DAVout[CH_EP_Cell]=DAVout[CH_EP_Cell];
	}
	if(CFPARA[CURNUM][5] > 0.0){
		if(fabs(e_sr-CFPARA[CURNUM][5]) >= ERR_StressA){
			DAVout[CH_EP_Cell]=DAVout[CH_EP_Cell]-float(0.1*DA_Cal_a[CH_EP_Cell]*(e_sr-CFPARA[CURNUM][5]));	// e_sr ballanced
		}
	}
	else {
		DAVout[CH_EP_Cell]=DAVout[CH_EP_Cell];
	...
	if(CFPARA[CURNUM][0]==0.0){
		...
		if(NUM_Cyclic!=0 && NUM_Cyclic <= CFPARA[CURNUM][4]){
			if(Flag_Cyclic==FALSE){
				DAVout[CH_MotorCruch]=0.0f;		// Cruch:Down
				if(eLDT>=CFPARA[CURNUM][3]) Flag_Cyclic=TRUE;
			}
			if(Flag_Cyclic==TRUE){
				DAVout[CH_MotorCruch]=5.0f;		// Cruch:Up
				if(eLDT<=CFPARA[CURNUM][2]) {
					Flag_Cyclic=FALSE;
					NUM_Cyclic=NUM_Cyclic+1;
				}
			}
		...
		if(NUM_Cyclic!=0 && NUM_Cyclic <= CFPARA[CURNUM][4]){
			...
			if(Flag_Cyclic==TRUE){
				DAVout[CH_MotorCruch]=5.0f;		// Cruch:Up
				if(eLDT<=CFPARA[CURNUM][2]) Flag_Cyclic=FALSE;
			}
		}
		if(NUM_Cyclic>CFPARA[CURNUM][4]){ 
			CURNUM=CURNUM+1;
